...
- Slack / Discord / Email / Signal の通知設定
- 通知ルール（重要度フィルタリング）
- テスト通知の送信

### インシデント通知設定

- クリティカルなシステムアラート（トークン期限切れ、オーケストレーターオフラインなど）のイベントごとのオン/オフ切り替え
- Email（SMTP または Firebase）、Webhook、Signal の各チャンネル
- 重複排除 — 各インシデントは解決されるまで 1 回のみ通知
